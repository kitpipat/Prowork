		...
		<nav>
			<ul class="xCNPagenation pagination justify-content-end">
				...
				<li class="page-item <?=$tDisabledLeft;?>">
					<a class="page-link" aria-label="Previous" onclick="JSvPOPDTTemp_ClickPage('previous')"><span aria-hidden="true">&laquo;</span></a>
				</li>
				...
				<?php for($i=max($nPage-2, 1); $i<=max(0, min($aListPDT['rnAllPage'],$nPage+2)); $i++){?>
					...
					?>
					<li class="page-item <?=$tActive;?> " onclick="JSvPOPDTTemp_ClickPage('<?=$i?>')"><a class="page-link"><?=$i?></a></li>
				<?php } ?>

				<!--ปุ่มไปต่อ-->
				<?php if($nPage >= $aListPDT['rnAllPage']){ $tDisabledRight = 'disabled'; }else{ $tDisabledRight = '-'; } ?>
				<li class="page-item <?=$tDisabledRight?>">
					<a class="page-link" aria-label="Next" onclick="JSvPOPDTTemp_ClickPage('next')"><span aria-hidden="true">&raquo;</span></a>
				</li>
			</ul>
		</nav>
    </div>
...
<script>
	...
	//เปลี่ยนหน้า
	function JSvPOPDTTemp_ClickPage(ptPage) {
		var nPageCurrent = '';
		switch (ptPage) {
		...
		JSxSelectPDTToTmp_PO(nPageCurrent);
	}
