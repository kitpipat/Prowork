...
<table class="table table-striped xCNTableCenter">
  <thead>
    <tr>
		...
		<th style="width:200px; text-align: left;">รหัสหน่วยสินค้า</th>
		...
		<?php if($aList['rtCode'] != 800){ ?>
			<?php foreach($aList['raItems'] AS $nKey => $aValue){ ?>
				<tr>
					...
					<td><img class="img-responsive xCNImageEdit" src="<?=base_url().'application/assets/images/icon/edit.png';?>" onClick="JSwUnitProductCallPageInsert('edit','<?=$aValue['FTPunCode']?>');"></td>
		...
		<nav>
			<ul class="xCNPagenation pagination justify-content-end">
				...
				<li class="page-item <?=$tDisabledLeft;?>">
					<a class="page-link" aria-label="Previous" onclick="JSvunitPDT_ClickPage('Fisrt')"><span aria-hidden="true">&laquo;</span></a>
				</li>
				<li class="page-item <?=$tDisabledLeft;?>">
					<a class="page-link" aria-label="Previous" onclick="JSvunitPDT_ClickPage('previous')"><span aria-hidden="true">&lsaquo;</span></a>
				</li>
				...
				<?php for($i=max($nPage-2, 1); $i<=max(0, min($aList['rnAllPage'],$nPage+2)); $i++){?>
					...
					?>
					<li class="page-item <?=$tActive;?> " onclick="JSvunitPDT_ClickPage('<?=$i?>')"><a class="page-link"><?=$i?></a></li>
				<?php } ?>

				<!--ปุ่มไปต่อ-->
				<?php if($nPage >= $aList['rnAllPage']){ $tDisabledRight = 'disabled'; }else{ $tDisabledRight = '-'; } ?>
				<li class="page-item <?=$tDisabledRight?>">
					<a class="page-link" aria-label="Next" onclick="JSvunitPDT_ClickPage('next')"><span aria-hidden="true">&rsaquo;</span></a>
				</li>
				<li class="page-item <?=$tDisabledRight?>">
					<a class="page-link" aria-label="Next" onclick="JSvunitPDT_ClickPage('Last')"><span aria-hidden="true">&raquo;</span></a>
				</li>
			</ul>
		</nav>
    </div>
...
<script>
	//เปลี่ยนหน้า
	function JSvunitPDT_ClickPage(ptPage) {
		var nPageCurrent = '';
		switch (ptPage) {
	...
	//ลบข้อมูล
	function JSxunitProduct_Delete(ptCode){
