...
<table class="table table-striped xCNTableCenter">
  ...
  <tbody>
		<?php if($aGroupList['rtCode'] != 800){ ?>
			<?php foreach($aGroupList['raItems'] AS $nKey => $aValue){ ?>
				<tr>
					...
					<?php 
						if( $aValue['PDT_use'] == '' || $aValue['PDT_use'] == null){
							$oEventDelete 			= "JSxGroupProduct_Delete('".$aValue['FTPgpCode']."')";
...
<div class="row">
    <div class="col-md-6">
        <label>พบข้อมูลทั้งหมด <?=$aGroupList['rnAllRow']?> รายการ แสดงหน้า <?=$aGroupList['rnCurrentPage']?> / <?=$aGroupList['rnAllPage']?></label>
    </div>
    <div class="col-md-6">
		<nav>
			<ul class="xCNPagenation pagination justify-content-end">
				...
				<li class="page-item <?=$tDisabledLeft;?>">
					<a class="page-link" aria-label="Previous" onclick="JSvProductGroup_ClickPage('Fisrt')"><span aria-hidden="true">&laquo;</span></a>
				</li>
				<li class="page-item <?=$tDisabledLeft;?>">
					<a class="page-link" aria-label="Previous" onclick="JSvProductGroup_ClickPage('previous')"><span aria-hidden="true">&lsaquo;</span></a>
				</li>

				<!--ปุ่มจำนวนหน้า-->
				<?php for($i=max($nPage-2, 1); $i<=max(0, min($aGroupList['rnAllPage'],$nPage+2)); $i++){?>
					<?php 
						if($nPage == $i){ 
							$tActive 		= 'active'; 
					...
					?>
					<li class="page-item <?=$tActive;?> " onclick="JSvProductGroup_ClickPage('<?=$i?>')"><a class="page-link"><?=$i?></a></li>
				<?php } ?>

				<!--ปุ่มไปต่อ-->
				<?php if($nPage >= $aGroupList['rnAllPage']){ $tDisabledRight = 'disabled'; }else{ $tDisabledRight = '-'; } ?>
				<li class="page-item <?=$tDisabledRight?>">
					<a class="page-link" aria-label="Next" onclick="JSvProductGroup_ClickPage('next')"><span aria-hidden="true">&rsaquo;</span></a>
				</li>
				<li class="page-item <?=$tDisabledRight?>">
					<a class="page-link" aria-label="Next" onclick="JSvProductGroup_ClickPage('Last')"><span aria-hidden="true">&raquo;</span></a>
				</li>
			</ul>
		</nav>
    </div>
...
<script>
	//เปลี่ยนหน้า
	function JSvProductGroup_ClickPage(ptPage) {
		var nPageCurrent = '';
		switch (ptPage) {
			...
			case 'Last': //กดหน้าสุดท้าย
				nPageCurrent 	= '<?=$aGroupList['rnAllPage']?>';
			break;
			default:
				nPageCurrent = ptPage
		}
	...
	//ลบข้อมูล
	function JSxGroupProduct_Delete(ptCode){
