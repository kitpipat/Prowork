		...
		<?php if($aSUPList['rtCode'] != 800){ ?>
			<?php foreach($aSUPList['raItems'] AS $nKey => $aValue){ ?>
				<tr>
					...
					<td class="<?=$tPer_delete?>"><img class="img-responsive xCNImageDelete" src="<?=base_url().'application/assets/images/icon/delete.png';?>" onClick="JSxSupplier_Delete('<?=$aValue['FTSplCode']?>');"></td>
				</tr>
			<?php } ?>
		<?php }else{ ?>
		...
		<nav>
			<ul class="xCNPagenation pagination justify-content-end">
				...
				<li class="page-item <?=$tDisabledLeft;?>">
					<a class="page-link" aria-label="Previous" onclick="JSvSupplier_ClickPage('Fisrt')"><span aria-hidden="true">&laquo;</span></a>
				</li>

				<li class="page-item <?=$tDisabledLeft;?>">
					<a class="page-link" aria-label="Previous" onclick="JSvSupplier_ClickPage('previous')"><span aria-hidden="true">&lsaquo;</span></a>
				</li>
				...
				<li class="page-item <?=$tDisabledRight?>">
					<a class="page-link" aria-label="Next" onclick="JSvSupplier_ClickPage('next')"><span aria-hidden="true">&rsaquo;</span></a>
				</li>

				<li class="page-item <?=$tDisabledRight?>">
					<a class="page-link" aria-label="Next" onclick="JSvSupplier_ClickPage('Last')"><span aria-hidden="true">&raquo;</span></a>
				</li>
			</ul>
		</nav>
    </div>
	...
	function JSvSupplier_ClickPage(ptPage) {
		...
		switch (ptPage) {
			...
			case 'Last': //กดหน้าสุดท้าย
				nPageCurrent 	= '<?=$aSUPList['rnAllPage']?>';
			break;
			default:
				nPageCurrent = ptPage
		}
